...
from pose_extraction.dwpose.util import (
    get_model_type,
    get_ort_providers,
...
class AnimalPoseImage:
    ...
    def __call__(self, oriImg):
        ...
        if is_model_torchscript(self.det):
            ...
            print(
                f"AnimalPose: Bbox {((torch_timer.timer() - det_start) * 1000):.2f}ms"
            )
        else:
        ...
        if is_model_torchscript(self.pose):
            ...
            print(
                f"AnimalPose: Pose {((torch_timer.timer() - pose_start) * 1000):.2f}ms on {det_result.shape[0]} animals\n"
            )
        else:
