    ...
    if not os.path.exists(model_path):
        ...
        if not os.path.exists(model_path):
            ...
            try:
                ...
                warnings.warn(
                    f"SSL verify failed, try use HTTP instead. {filename}'s hash will be checked"
                )
                download_url_to_file(url=model_url, dst=model_path)
                assert check_hash_from_torch_hub(
                    model_path, filename
...
def custom_hf_download(
    ...
    if not os.path.exists(model_path):
        ...
        if use_symlinks:
            ...
            )  # use huggingface newer env variables `HF_HUB_CACHE`
            if cache_dir_d is None:
                import platform

                if platform.system() == "Windows":
                    cache_dir_d = os.path.join(
                        os.getenv("USERPROFILE"), ".cache", "huggingface", "hub"
                    )
                else:
                    cache_dir_d = os.path.join(
                        os.getenv("HOME"), ".cache", "huggingface", "hub"
                    )
            try:
                # test_link
                ...
                print(
                    "Using symlinks to download models. \n",
